...
void filter_phi_d_exc_recon(string reaction, string output_mode)
{
    ...
    .Define("kp_phi_meas",                      "kp_p4_meas .Phi()  *RadToDeg")
    .Define("kp_phi_kin",                       "kp_p4_kin  .Phi()  *RadToDeg")
    ...
    .Define("km_phi_meas",                      "km_p4_meas .Phi()  *RadToDeg")
    .Define("km_phi_kin",                       "km_p4_kin  .Phi()  *RadToDeg")
    ...
    if (output_mode == "hist" || output_mode == "both")
    {
        ...
        for (int i = 0; i < N_filters; i++)
        {
            ...
            if (reaction.find("sim") != string::npos)
            {
                ...
                TH2D    hist_resolution_decay_costheta                  = *rdf.Histo2D({("resolution_decay_costheta_"+ label).c_str(),          ";cos(#theta_{helicity}^{truth})        ;cos(#theta_{helicity}^{kin}) - cos(#theta_{helicity}^{truth})",    40,-1.0, 1.0, 80, -1.0, 1.0},           "decay_costheta_helicity_truth",    "decay_costheta_helicity_diff", "event_weight");
                        hist_resolution_decay_costheta.Write();
                TH2D    hist_resolution_decay_phi                       = *rdf.Histo2D({("resolution_decay_phi_"+ label).c_str(),               ";#phi_{helicity}^{truth} (deg)         ;#phi_{helicity}^{kin} - #phi_{helicity}^{truth} (deg)",            40, -180.0, 180.0, 80, -8.0, 8.0},      "decay_phi_helicity_truth",         "decay_phi_helicity_diff",      "event_weight");
                        hist_resolution_decay_phi.Write();
                TH2D    hist_resolution_polarization_phi                = *rdf.Histo2D({("resolution_polarization_phi_"+ label).c_str(),        ";#Phi_{pol}^{truth} (deg)              ;#Phi_{pol}^{kin} - #Phi_{pol}^{truth} (deg)",                      40, -180.0, 180.0, 80, -2.0, 2.0},      "polarization_phi_com_truth",       "polarization_phi_com_diff",    "event_weight");
                        hist_resolution_polarization_phi.Write();
                TH2D    hist_resolution_diff_psi                        = *rdf.Histo2D({("resolution_diff_psi_"+ label).c_str(),                ";#psi_{helicity}^{truth} (deg)         ;#psi_{helicity}^{kin} - #psi_{helicity}^{truth} (deg)",            40, -180.0, 180.0, 80, -8.0, 8.0},      "diff_psi_helicity_truth",          "diff_psi_helicity_diff",       "event_weight");
                        hist_resolution_diff_psi.Write();
                TH2D    hist_resolution_sum_psi                         = *rdf.Histo2D({("resolution_sum_psi_"+ label).c_str(),                 ";#Psi_{helicity}^{truth} (deg)         ;#Psi_{helicity}^{kin} - #Psi_{helicity}^{truth} (deg)",            40, -180.0, 180.0, 80, -8.0, 8.0},      "sum_psi_helicity_truth",           "sum_psi_helicity_diff",         "event_weight");
