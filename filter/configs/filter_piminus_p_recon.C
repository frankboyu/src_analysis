...
void filter_piminus_p_recon(string reaction_name, string input_mode, string output_mode)
{
    ...
    if (output_mode == "hist" || output_mode == "both")
    {
        ...
        for (int i = 0; i < N_filters; i++)
        {
            ...
            TH1D hist_beam_DeltaT = *rdf.Histo1D({("beam_DeltaT_"+ label).c_str(), ";#Delta t_{beam} (ns);Counts", 400, -20.0, 20.0},"beam_DeltaT_kin");
            ...
            TH2D hist_pim_dEdx_cdc = *rdf.Histo2D({("pim_dEdx_cdc_"+ label).c_str(), ";p (GeV/c);dE/dx (keV/cm)", 100, 0.0, 10.0, 400, 0.0, 40},"pim_momentum_kin","pim_dedx_cdc_keV_per_cm","accidweight");
            hist_pim_dEdx_cdc.Write();
            TH2D hist_pim_dEdx_fdc = *rdf.Histo2D({("pim_dEdx_fdc_"+ label).c_str(), ";p (GeV/c);dE/dx (keV/cm)", 100, 0.0, 10.0, 400, 0.0, 40},"pim_momentum_kin","pim_dedx_fdc_keV_per_cm","accidweight");
            hist_pim_dEdx_fdc.Write();
            TH2D hist_pim_dEdx_tof = *rdf.Histo2D({("pim_dEdx_tof_"+ label).c_str(), ";p (GeV/c);dE/dx (keV/cm)", 100, 0.0, 10.0, 400, 0.0, 40},"pim_momentum_kin","pim_dedx_tof_keV_per_cm","accidweight");
            hist_pim_dEdx_tof.Write();
            TH2D hist_pim_dEdx_st = *rdf.Histo2D({("pim_dEdx_st_"+ label).c_str(), ";p (GeV/c);dE/dx (keV/cm)", 100, 0.0, 10.0, 400, 0.0, 40},"pim_momentum_kin","pim_dedx_st_keV_per_cm","accidweight");
            hist_pim_dEdx_st.Write();
            TH1D hist_pim_DeltaT = *rdf.Histo1D({("pim_DeltaT_"+ label).c_str(), ";#Delta t_{#pi^{-}} (ns);Counts", 400, -20.0, 20.0},"pim_DeltaT_kin","accidweight");
            hist_pim_DeltaT.Write();
            TH2D hist_pim_kinematics = *rdf.Histo2D({("pim_kinematics_"+ label).c_str(), ";P_{#pi^{-}} (GeV/c);#theta_{#pi^{-}} (deg)", 100, 0.0, 10.0, 180, 0.0, 180.0},"pim_momentum_kin","pim_theta_kin","accidweight");
            hist_pim_kinematics.Write();
            TH2D hist_pim_kinematics_fdc = *rdf.Histo2D({("pim_kinematics_fdc_"+ label).c_str(), ";P_{#pi^{-}} (GeV/c);#theta_{#pi^{-}} (deg)", 100, 0.0, 10.0, 180, 0.0, 180.0},"pim_momentum_kin","pim_theta_kin","pim_in_fdc");
            hist_pim_kinematics_fdc.Write();
            TH2D hist_pim_kinematics_fdc_cdc = *rdf.Histo2D({("pim_kinematics_fdc_cdc_"+ label).c_str(), ";P_{#pi^{-}} (GeV/c);#theta_{#pi^{-}} (deg)", 100, 0.0, 10.0, 180, 0.0, 180.0},"pim_momentum_kin","pim_theta_kin","pim_in_fdc_cdc");
            hist_pim_kinematics_fdc_cdc.Write();
            TH2D hist_pim_kinematics_cdc = *rdf.Histo2D({("pim_kinematics_cdc_"+ label).c_str(), ";P_{#pi^{-}} (GeV/c);#theta_{#pi^{-}} (deg)", 100, 0.0, 10.0, 180, 0.0, 180.0},"pim_momentum_kin","pim_theta_kin","pim_in_cdc");
            hist_pim_kinematics_cdc.Write();
            TH2D hist_pim_kinematics_neither = *rdf.Histo2D({("pim_kinematics_neither_"+ label).c_str(), ";P_{#pi^{-}} (GeV/c);#theta_{#pi^{-}} (deg)", 100, 0.0, 10.0, 180, 0.0, 180.0},"pim_momentum_kin","pim_theta_kin","pim_in_neither");
            hist_pim_kinematics_neither.Write();

            TH1D hist_p_pidfom = *rdf.Histo1D({("p_pidfom_"+ label).c_str(), ";p_pidfom;Counts", 100, 0.0, 1.0},"p_pidfom","accidweight");
            hist_p_pidfom.Write();
            TH2D hist_p_dEdx_cdc = *rdf.Histo2D({("p_dEdx_cdc_"+ label).c_str(), ";p (GeV/c);dE/dx (keV/cm)", 100, 0.0, 10.0, 400, 0.0, 40},"p_momentum_kin","p_dedx_cdc_keV_per_cm","accidweight");
            hist_p_dEdx_cdc.Write();
            TH2D hist_p_dEdx_fdc = *rdf.Histo2D({("p_dEdx_fdc_"+ label).c_str(), ";p (GeV/c);dE/dx (keV/cm)", 100, 0.0, 10.0, 400, 0.0, 40},"p_momentum_kin","p_dedx_fdc_keV_per_cm","accidweight");
            hist_p_dEdx_fdc.Write();
            TH2D hist_p_dEdx_tof = *rdf.Histo2D({("p_dEdx_tof_"+ label).c_str(), ";p (GeV/c);dE/dx (keV/cm)", 100, 0.0, 10.0, 400, 0.0, 40},"p_momentum_kin","p_dedx_tof_keV_per_cm","accidweight");
            hist_p_dEdx_tof.Write();
            TH2D hist_p_dEdx_st = *rdf.Histo2D({("p_dEdx_st_"+ label).c_str(), ";p (GeV/c);dE/dx (keV/cm)", 100, 0.0, 10.0, 400, 0.0, 40},"p_momentum_kin","p_dedx_st_keV_per_cm","accidweight");
            hist_p_dEdx_st.Write();
            TH1D hist_p_DeltaT = *rdf.Histo1D({("p_DeltaT_"+ label).c_str(), ";#Delta t_{p} (ns);Counts", 400, -20.0, 20.0},"p_DeltaT_kin","accidweight");
            hist_p_DeltaT.Write();
            TH2D hist_p_kinematics = *rdf.Histo2D({("p_kinematics_"+ label).c_str(), ";P_{p} (GeV/c);#theta_{p} (deg)", 100, 0.0, 10.0, 180, 0.0, 180.0},"p_momentum_kin","p_theta_kin","accidweight");
            hist_p_kinematics.Write();
            TH2D hist_p_kinematics_fdc = *rdf.Histo2D({("p_kinematics_fdc_"+ label).c_str(), ";P_{p} (GeV/c);#theta_{p} (deg)", 100, 0.0, 10.0, 180, 0.0, 180.0},"p_momentum_kin","p_theta_kin","p_in_fdc");
            hist_p_kinematics_fdc.Write();
            TH2D hist_p_kinematics_fdc_cdc = *rdf.Histo2D({("p_kinematics_fdc_cdc_"+ label).c_str(), ";P_{p} (GeV/c);#theta_{p} (deg)", 100, 0.0, 10.0, 180, 0.0, 180.0},"p_momentum_kin","p_theta_kin","p_in_fdc_cdc");
            hist_p_kinematics_fdc_cdc.Write();
            TH2D hist_p_kinematics_cdc = *rdf.Histo2D({("p_kinematics_cdc_"+ label).c_str(), ";P_{p} (GeV/c);#theta_{p} (deg)", 100, 0.0, 10.0, 180, 0.0, 180.0},"p_momentum_kin","p_theta_kin","p_in_cdc");
            hist_p_kinematics_cdc.Write();
            TH2D hist_p_kinematics_neither = *rdf.Histo2D({("p_kinematics_neither_"+ label).c_str(), ";P_{p} (GeV/c);#theta_{p} (deg)", 100, 0.0, 10.0, 180, 0.0, 180.0},"p_momentum_kin","p_theta_kin","p_in_neither");
            hist_p_kinematics_neither.Write();

            TH2D hist_pim_p_theta = *rdf.Histo2D({("pim_p_theta_"+ label).c_str(), ";#theta_{#pi^{-}} (deg);#theta_{p} (deg)", 180, 0.0, 180.0, 180, 0.0, 180.0},"pim_theta_kin","p_theta_kin","accidweight");
            hist_pim_p_theta.Write();
            TH2D hist_pim_p_momentum = *rdf.Histo2D({("pim_p_momentum_"+ label).c_str(), ";P_{#pi^{-}} (GeV/c);P_{p} (GeV/c)", 100, 0.0, 10.0, 100, 0.0, 10.0},"pim_momentum_kin","p_momentum_kin","accidweight");
            hist_pim_p_momentum.Write();

            TH1D hist_sqrts = *rdf.Histo1D({("sqrts_"+ label).c_str(), ";#sqrt{s} (GeV);Counts", 100, 0.0, 10.0},"sqrts_kin","accidweight");
            hist_sqrts.Write();
            TH1D hist_minust = *rdf.Histo1D({("minust_"+ label).c_str(), ";-t (GeV^{2}/c^{2});Counts", 200, 0.0, 20.0},"minust_kin","accidweight");
            hist_minust.Write();
            ...
            hist_thetaCM.Write();
            TH2D hist_minust_thetaCM = *rdf.Histo2D({("minust_thetaCM_"+ label).c_str(), ";-t (GeV^{2}/c^{2});#theta_{CM} (deg)", 200, 0.0, 20.0, 180, 0.0, 180.0},"minust_kin","thetaCM_kin","accidweight");
            hist_minust_thetaCM.Write();
            TH2D hist_thetaCM_recon_truth = *rdf.Histo2D({("thetaCM_recon_truth_"+ label).c_str(), ";#theta_{CM}^{recon} (deg);#theta_{CM}^{truth} (deg)", 36, 0.0, 180.0, 36, 0.0, 180.0},"thetaCM_kin","thetaCM_truth","accidweight");
            hist_thetaCM_recon_truth.Write();
            TH1D hist_rho_mass = *rdf.Histo1D({("rho_mass_"+ label).c_str(), ";m_{#pi^{+}#pi^{-}} (GeV/c^{2});Counts", 400, 0.0, 4.0},"rho_mass_kin","accidweight");
            hist_rho_mass.Write();

            TH1D hist_n_mass = *rdf.Histo1D({("n_mass_"+ label).c_str(), ";m_{n} (GeV/c^{2});Counts", 400, -4.0, 4.0},"n_mass_kin","accidweight");
            hist_n_mass.Write();
            TH1D hist_n_mass_truth = *rdf.Histo1D({("n_mass_truth_"+ label).c_str(), ";m_{n} (GeV/c^{2});Counts", 100, 0.0, 4.0},"n_mass_truth","accidweight");
            hist_n_mass_truth.Write();
            TH1D hist_n_momentum = *rdf.Histo1D({("n_momentum_"+ label).c_str(), ";P_{n} (GeV/c);Counts", 100, 0.0, 1.0},"n_momentum_kin","accidweight");
            hist_n_momentum.Write();
            TH1D hist_n_momentum_truth = *rdf.Histo1D({("n_momentum_truth_"+ label).c_str(), ";P_{n} (GeV/c);Counts", 100, 0.0, 4.0},"n_momentum_truth","accidweight");
            hist_n_momentum_truth.Write();
            TH1D hist_n_pminus = *rdf.Histo1D({("n_pminus_"+ label).c_str(), ";P_{n}^{-} (GeV/c);Counts", 120, 0.3, 1.5},"n_pminus_kin","accidweight");
            hist_n_pminus.Write();
            TH1D hist_n_pminus_truth = *rdf.Histo1D({("n_pminus_truth_"+ label).c_str(), ";P_{n}^{-} (GeV/c);Counts", 100, 0.4, 1.4},"n_pminus_truth","accidweight");
            hist_n_pminus_truth.Write();
            TH1D hist_n_energy_balance = *rdf.Histo1D({("n_energy_balance_"+ label).c_str(), ";E_{n} - m_{n} (GeV);Counts", 400, -4.0, 4.0},"n_energy_balance_kin","accidweight");
            hist_n_energy_balance.Write();
            TH1D hist_n_energy_balance_truth = *rdf.Histo1D({("n_energy_balance_truth_"+ label).c_str(), ";E_{n} - m_{n} (GeV);Counts", 400, -4.0, 4.0},"n_energy_balance_truth","accidweight");
            hist_n_energy_balance_truth.Write();
            TH2D hist_n_momentum_pminus = *rdf.Histo2D({("n_momentum_pminus_"+ label).c_str(), ";P_{n} (GeV/c);P_{n}^{-} (GeV/c)", 100, 0.0, 1.0, 100, 0.4, 1.4},"n_momentum_kin","n_pminus_kin","accidweight");
            hist_n_momentum_pminus.Write();

            TH1D hist_N2miss_mass = *rdf.Histo1D({("N2miss_mass_"+ label).c_str(), ";m_{n} (GeV/c^{2});Counts", 400, -4.0, 4.0},"N2miss_mass_kin","accidweight");
            hist_N2miss_mass.Write();
            TH1D hist_N2miss_mass_truth = *rdf.Histo1D({("N2miss_mass_truth_"+ label).c_str(), ";m_{n} (GeV/c^{2});Counts", 100, 0.0, 4.0},"N2miss_mass_truth","accidweight");
            hist_N2miss_mass_truth.Write();
            TH1D hist_N2miss_momentum = *rdf.Histo1D({("N2miss_momentum_"+ label).c_str(), ";P_{n} (GeV/c);Counts", 100, 0.0, 1.0},"N2miss_momentum_kin","accidweight");
            hist_N2miss_momentum.Write();
            TH1D hist_N2miss_momentum_truth = *rdf.Histo1D({("N2miss_momentum_truth_"+ label).c_str(), ";P_{n} (GeV/c);Counts", 100, 0.0, 4.0},"N2miss_momentum_truth","accidweight");
            hist_N2miss_momentum_truth.Write();
            TH1D hist_N2miss_pminus = *rdf.Histo1D({("N2miss_pminus_"+ label).c_str(), ";P_{n}^{-} (GeV/c);Counts", 120, 0.3, 1.5},"N2miss_pminus_kin","accidweight");
            hist_N2miss_pminus.Write();
            TH1D hist_N2miss_pminus_truth = *rdf.Histo1D({("N2miss_pminus_truth_"+ label).c_str(), ";P_{n}^{-} (GeV/c);Counts", 100, 0.4, 1.4},"N2miss_pminus_truth","accidweight");
            hist_N2miss_pminus_truth.Write();
            TH1D hist_N2miss_energy_balance = *rdf.Histo1D({("N2miss_energy_balance_"+ label).c_str(), ";E_{n} - m_{n} (GeV);Counts", 400, -4.0, 4.0},"N2miss_energy_balance_kin","accidweight");
            hist_N2miss_energy_balance.Write();
            TH1D hist_N2miss_energy_balance_truth = *rdf.Histo1D({("N2miss_energy_balance_truth_"+ label).c_str(), ";E_{n} - m_{n} (GeV);Counts", 400, -4.0, 4.0},"N2miss_energy_balance_truth","accidweight");
            hist_N2miss_energy_balance_truth.Write();
            TH2D hist_N2miss_momentum_pminus = *rdf.Histo2D({("N2miss_momentum_pminus_"+ label).c_str(), ";P_{n} (GeV/c);P_{n}^{-} (GeV/c)", 100, 0.0, 1.0, 100, 0.4, 1.4},"N2miss_momentum_kin","N2miss_pminus_kin","accidweight");
            hist_N2miss_momentum_pminus.Write();
        }
        hist_file->Close();
    }
    cout << "Done!\n";
