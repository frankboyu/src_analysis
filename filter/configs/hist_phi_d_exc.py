...
file_data   = File("/work/halld2/home/boyu/src_analysis/filter/output/vm_d_20260428/filteredhist_phi_d_recon_exc_data_2H_ver12.root")
file_sim    = File("/work/halld2/home/boyu/src_analysis/filter/output/vm_d_20260311/filteredhist_phi_d_recon_exc_sim_2H_ver12_flat.root")
file_tagged = File("/work/halld2/home/boyu/src_analysis/filter/output/vm_d_20260311/filteredhist_phi_d_thrown_exc_tagged_2H_ver12_flat.root")
file_gen    = File("/work/halld2/home/boyu/src_analysis/filter/output/vm_d_20260311/filteredhist_phi_d_thrown_exc_gen_2H_ver12_flat.root")

###################################################################### CUTS RELATED #####################################################################################

print("deuteron CDC dE/dx")
# deuteron CDC dE/dx: comparison between data and simulation
...
# deuteron CDC dE/dx: fit data with double Gaussian

# deuteron CDC dE/dx: fit to get exponential cuts with different significances

print("Missing p minus")
# Missing p minus: comparison between data and simulation
...
# Missing p minus: fit data with double Gaussian
...
plt.text(-0.19, 600, f"$A_1$: {popt[0]:.4f}\n$\mu_1$: {popt[1]:.4f} GeV/c\n$\sigma_1$: {popt[2]:.4f} GeV/c", color='blue')
plt.text(-0.19, 500, f"$A_2$: {popt[3]:.4f}\n$\mu_2$: {popt[4]:.4f} GeV/c\n$\sigma_2$: {popt[5]:.4f} GeV/c", color='green')
...
# Missing p minus: find optimized cut value by maximizing the signal significance
fig = plt.figure(figsize=(8, 6), dpi=300)
cut_values = np.linspace(-0.1, 0.0, 100)
significance = np.zeros_like(cut_values)
for i, cut in enumerate(cut_values):
    signal = abs(quad(lambda x: gaussian(x, popt[3], popt[4], popt[5]), cut, 0.02)[0])
    background = abs(quad(lambda x: gaussian(x, popt[0], popt[1], popt[2]), cut, 0.02)[0])
    significance[i] = signal/np.sqrt(signal+background)
plt.plot(cut_values, significance, '-o', color='red')
plt.plot([-0.02, -0.02], [2, 4], '-', color='red')
...
plt.plot(np.linspace(0,10,1000), 4000*scipy.stats.chi2.pdf(np.linspace(0,10,1000)*7, df=7)*7, '-', color='red', label=r'$\chi^2$ PDF (ndf=7)')
