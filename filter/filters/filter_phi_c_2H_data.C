...
{
    string InputFile  = "/work/halld2/home/boyu/src_analysis/selection/output/flattree_phi_c_2H_data_ver02/*.root";
    ...
    .Define("miss_p4_meas","beam_p4_meas + target_p4 - kp_p4_meas - km_p4_meas")
    .Define("miss_p4_kin","beam_p4_kin + target_p4 - kp_p4_kin - km_p4_kin")
    ...
    .Define("minus_u_meas", "-(beam_p4_meas - miss_p4_meas).Mag2()")
    .Define("minus_u_kin", "-(beam_p4_kin - miss_p4_kin).Mag2()")
    .Define("rho_mass_meas","(kp_as_pip_p4_meas + km_as_pim_p4_meas).M()")
    .Define("rho_mass_kin","(kp_as_pip_p4_kin + km_as_pim_p4_kin).M()")
    .Define("y_phi_meas","minus_t_meas/(2*mass_target*(beam_p4_meas.E()-phi_p4_meas.E()))")
    .Define("y_phi_kin","minus_t_kin/(2*mass_target*(beam_p4_kin.E()-phi_p4_kin.E()))")
    .Define("deltaE_meas","(pow(sqrt_s_meas, 2) - pow(mass_target, 2) + pow(mass_phi, 2)) / (2.0 * sqrt_s_meas) - phi_p4com_meas.E()")
    .Define("deltaE_kin","(pow(sqrt_s_kin, 2) - pow(mass_target, 2) + pow(mass_phi, 2)) / (2.0 * sqrt_s_kin) - phi_p4com_kin.E()")
    ;
    ...
    for (int i = 0; i < N_filters; i++)
    {
        ...
        hist_massKK.Write();
        TH1D hist_massmiss = *rdf.Histo1D({("massmiss_"+ label).c_str(), ";m_{miss} (GeV/c^{2});Counts",400, 0.0, 4.0},"miss_mass_meas","accidweight");
        hist_massmiss.Write();
        TH1D hist_pMinusMiss = *rdf.Histo1D({("pMinusMiss_"+ label).c_str(), ";p_{miss}^{-} (GeV/c);Counts", 100, 1.0, 3.0},"miss_pMinus_meas","accidweight");
        hist_pMinusMiss.Write();
        ...
        TH2D hist_massmiss_yphi = *rdf.Histo2D({("massmiss_yphi_"+ label).c_str(), ";m_{miss} (GeV/c^{2});y_{#phi}", 400, 0.0, 4.0, 200, 0.0, 2.0},"miss_mass_meas","y_phi_meas","accidweight");
        hist_massmiss_yphi.Write();
        TH2D hist_pMinusMiss_yphi = *rdf.Histo2D({("pMinusMiss_yphi_"+ label).c_str(), ";p_{miss}^{-} (GeV/c);y_{#phi}", 100, 1.0, 3.0, 200, 0.0, 2.0},"miss_pMinus_meas","y_phi_meas","accidweight");
