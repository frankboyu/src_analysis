...
vector<const char*> getBranches(TTree *tree)
{
    ...
    for (int i = 0; i < arr->GetEntries(); ++i)
    {
        ...
        if ( strstr( b_name, "__ChargedIndex" ) != NULL || strstr( b_name, "__NeutralIndex" ) != NULL )
        {
...
bool compareMaps( map<string, Int_t> p_map, map<string, Int_t> s_map )
{
   ...
   for ( auto const &p_iter : p_map )
   {
      ...
      for ( auto const &s_iter : s_map )
      {
        ...
        {
	 	if( (p_iter.first.find("Plus")!=string::npos) && (s_iter.first.find("Plus")!=string::npos) )
        {
			matchedParticles++;
		}
		else if( (p_iter.first.find("Minus")!=string::npos) && (s_iter.first.find("Minus")!=string::npos) ){
			matchedParticles++;
		}
		else if( (p_iter.first.find("Proton")!=string::npos) && (s_iter.first.find("Proton")!=string::npos) ){
			matchedParticles++;
		}
		else if( (p_iter.first.find("Photon")!=string::npos) && (s_iter.first.find("Photon")!=string::npos) ){
			matchedParticles++;
		}
	 }
      ...
      if(totalParticles != matchedParticles){
...
void mergeTrees(const char* primaryFile, const char* primaryTree, const char* secondaryFile, const char* secondaryTree)
{
   ...
   cout<<""<<endl;
   cout<<"Total num of  combo is "<<total_combo<<endl;
   cout<<"Matched num of combo is "<<matched_combo<<endl;
}
