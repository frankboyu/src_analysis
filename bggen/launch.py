...
def add_job(WORKFLOW, FILEPATH, config_dict):
	...
	match = re.search(r"(.*)/(.*)_(\d\d\d\d\d\d)_(\d\d\d)_(.*)\.(.*)", FILEPATH)
	if(match is not None):
		...
		match = re.search(r"(.*)/(.*)_(\d\d\d\d\d\d)_(.*)\.(.*)", FILEPATH)
		if(match is None):
	...
	if(VERBOSE == True):
		print("FILEPATH, COMPONENTS: " + FILEPATH + " " + INDATA_DIR + " " + PREFIX + " " + RUNNO + " " + FILENO + " " + SUFFIX + " " + EXTENSION)

	# PREPARE NAMES
	DATE = time.strftime("%Y-%m-%d")
	STUBNAME = RUNNO if(FILENO == "-1") else RUNNO + "_" + FILENO
	FILENAME = PREFIX + "_" + RUNNO + "_" + SUFFIX + "." + EXTENSION if(FILENO == "-1") else PREFIX + "_" + RUNNO + "_" + FILENO + "_" + SUFFIX + "." + EXTENSION
