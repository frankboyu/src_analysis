...
REACTION=$1
EVENTS=$2
WRAPPER_VERSION=gluex_MCwrapper_srcct-v2.13.0
...
do
    echo ${run} ${number}
    gluex_MC.py configs/wrapper_${REACTION}_${run}.cfg ${run} ${number} per_file=100000 batch=1 logdir=/farm_out/boyu/src_analysis/sim/${REACTION}
done < list.txt
