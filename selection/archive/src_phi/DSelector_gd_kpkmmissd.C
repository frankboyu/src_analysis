#include "DSelector_gd_kpkmmissd.h"

double RadToDeg = 180.0/3.1415926;

void DSelector_gd_kpkmmissd::Init(TTree *locTree)
{    
	//SET OUTPUT FILE NAME
	dOutputFileName     = "hist_gd_kpkmmissd.root"; //"" for none
    ...
    dHist_InvariantMass_Measured_Before           = new TH1F("InvariantMass_Measured_Before",          ";Invariant Mass (K^+K^-)        ;Events/0.01 GeV",     200,  0.0,   2.0);
    ...
    dHist_InvariantMass_Measured_After            = new TH1F("InvariantMass_Measured_After",           ";Invariant Mass (K^+K^-)        ;Events/0.01 GeV",     200,  0.0,   2.0);
    dHist_InvariantMass_KinFit_After              = new TH1F("InvariantMass_KinFit_After",             ";Invariant Mass (K^+K^-)        ;Events/0.01 GeV",     200,  0.0,   2.0);
    ...
    dHist_MissingDeuteronPVsTheta_Measured_After  = new TH2F("MissingDeuteronPVsTheta_Measured_After", ";P (GeV)                        ;#theta (deg)",       1000,  0.0, 10.0,   180,  0.0, 180.0);
    dHist_MissingDeuteronPVsTheta_KinFit_After    = new TH2F("MissingDeuteronPVsTheta_KinFit_After",   ";P (GeV)                        ;#theta (deg)",       1000,  0.0, 10.0,   180,  0.0, 180.0);

} 
//END OF INITIALIZATION

Bool_t DSelector_gd_kpkmmissd::Process(Long64_t locEntry)
{    
	//CALL THIS FIRST
	...
	for(UInt_t loc_i = 0; loc_i < Get_NumCombos(); ++loc_i)
    {
        ...
        TLorentzVector locMissingDeuteronP4          = locBeamP4 + dTargetP4 - locKPlusP4 - locKMinusP4;
        ...
        TLorentzVector locMissingDeuteronP4_Measured = locBeamP4_Measured + dTargetP4 - locKPlusP4_Measured - locKMinusP4_Measured;
        ...
        double locMinusU_Measured              = -(locBeamP4_Measured - locMissingDeuteronP4_Measured).Mag2();
        double locMinusU_KinFit                = -(locBeamP4 - locMissingDeuteronP4).Mag2();
        double locCoplanarity_Measured         = abs((locKPlusP4_Measured + locKMinusP4_Measured).Phi() - locMissingDeuteronP4_Measured.Phi())*RadToDeg;        
        double locCoplanarity_KinFit           = abs((locKPlusP4 + locKMinusP4).Phi() - locMissingDeuteronP4.Phi())*RadToDeg;
        ...
        dHist_MissingDeuteronPVsTheta_Measured_After ->Fill(locMissingDeuteronP4_Measured.P(), locMissingDeuteronP4_Measured.Theta()*RadToDeg, locHistAccidWeightFactor);    
        dHist_MissingDeuteronPVsTheta_KinFit_After   ->Fill(locMissingDeuteronP4.P(),          locMissingDeuteronP4.Theta()*RadToDeg,          locHistAccidWeightFactor);
...
void DSelector_gd_kpkmmissd::Finalize(void)
{
	//CALL THIS LAST
	DSelector::Finalize();
} //end of finalization