...
class DSelector_phi_d_misskm_recon : public DSelector
{
public:

    DSelector_phi_d_misskm_recon(TTree* locTree = NULL) : DSelector(locTree){}
    virtual ~DSelector_phi_d_misskm_recon(){}
...
private:
    ...
    ClassDef(DSelector_phi_d_misskm_recon, 0);
};

void DSelector_phi_d_misskm_recon::Get_ComboWrappers(void)
{
...
void DSelector_phi_d_misskm_recon::Init(TTree *locTree)
{
    ...
    dOutputFileName          = Form("selectedhist_phi_d_misskm_recon_%s.root", dTag.c_str());
    dOutputTreeFileName      = "";
    dFlatTreeFileName        = Form("selectedtree_phi_d_misskm_recon_%s.root", dTag.c_str());
    dFlatTreeName            = "selectedtree_phi_d_misskm_recon";
...
Bool_t DSelector_phi_d_misskm_recon::Process(Long64_t locEntry)
{
	// CALL THIS FIRST
    ...
    for(const auto& loc_combo : loc_combos)
	{
        ...
        TLorentzVector locKMinusP4      = locBeamP4 + TLorentzVector(0, 0, 0, 1.875612859) - locKPlusP4 - locDeuteronP4; // missing km
        ...
        TLorentzVector locKMinusP4_Measured     = locBeamP4_Measured + TLorentzVector(0, 0, 0, 1.875612859) - locKPlusP4_Measured - locDeuteronP4_Measured; // missing km
...
void DSelector_phi_d_misskm_recon::Finalize(void)
{
	// CALL THIS LAST
