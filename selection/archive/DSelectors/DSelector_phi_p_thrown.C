...
class DSelector_phi_p_thrown : public DSelector
{
public:

    DSelector_phi_p_thrown(TTree* locTree = NULL) : DSelector(locTree){}
    virtual ~DSelector_phi_p_thrown(){}
...
private:
    ...
    ClassDef(DSelector_phi_p_thrown, 0);
};

void DSelector_phi_p_thrown::Init(TTree *locTree)
{
    // SET OUTPUT FILE NAME
    dOutputFileName          = "";
    dOutputTreeFileName      = "";
    dFlatTreeFileName        = "selectedtree_phi_p_thrown.root";
    dFlatTreeName            = "selectedtree_phi_p_thrown";
...
Bool_t DSelector_phi_p_thrown::Process(Long64_t locEntry)
{
	// CALL THIS FIRST
...
void DSelector_phi_p_thrown::Finalize(void)
{
	// CALL THIS LAST
