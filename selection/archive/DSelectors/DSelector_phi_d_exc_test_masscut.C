...
class DSelector_phi_d_exc_test_masscut : public DSelector
{
public:

    DSelector_phi_d_exc_test_masscut(TTree* locTree = NULL) : DSelector(locTree){}
    virtual ~DSelector_phi_d_exc_test_masscut(){}
...
private:
    ...
    // CUSTOM HISTOGRAMS
    TH2D* dHist_2Kmass_2pimass;

    ClassDef(DSelector_phi_d_exc_test_masscut, 0);
};

void DSelector_phi_d_exc_test_masscut::Get_ComboWrappers(void)
{
...
void DSelector_phi_d_exc_test_masscut::Init(TTree *locTree)
{
    // SET OUTPUT FILE NAME
    dOutputFileName          = "selectedhist_phi_d_exc_test_masscut.root";
    dOutputTreeFileName      = "";
    dFlatTreeFileName        = "";
    dFlatTreeName            = "";
    ...
    // CUSTOM HISTOGRAMS
    dHist_2Kmass_2pimass     = new TH2D("hist_2Kmass_2pimass", ";m_{K^{+}K^{-}} (GeV/c^{2});m_{#pi^{+}#pi^{-}} (GeV/c^{2})", 100, 0.9, 1.5, 100, 0.0, 1.5);
}
// END OF INITIALIZATION

Bool_t DSelector_phi_d_exc_test_masscut::Process(Long64_t locEntry)
{
	// CALL THIS FIRST
    ...
    for(const auto& loc_combo : loc_combos)
	{
        ...
        dHist_2Kmass_2pimass     ->Fill((locKPlusP4+locKMinusP4).M(), (locKPlusP4_Measured_PionHypothesis+locKMinusP4_Measured_PionHypothesis).M(), locHistAccidWeightFactor*locComboAccidWeightFactor);
...
void DSelector_phi_d_exc_test_masscut::Finalize(void)
{
	// CALL THIS LAST
