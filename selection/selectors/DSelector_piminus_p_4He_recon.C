#include "DSelector_piminus_p_4He_recon.h"

double RadToDeg = 180.0/3.1415926;

void DSelector_piminus_p_4He_recon::Init(TTree *locTree)
{
	// SET OUTPUT FILE NAME
	dOutputFileName          = "";                                 
	dOutputTreeFileName      = "";                                 
	dFlatTreeFileName        = "flattree_piminus_p_4He_recon.root";
	dFlatTreeName            = "flattree_piminus_p_4He_recon";              
...
Bool_t DSelector_piminus_p_4He_recon::Process(Long64_t locEntry)
{
	// CALL THIS FIRST
	...
	for(UInt_t loc_i = 0; loc_i < Get_NumCombos(); ++loc_i)
	{
        ...
        // PERFORM THE CUTS
        if(dComboWrapper->Get_NumUnusedTracks()        > 5)                                                                         locCutFlag_NumUnusedTracks  = true;
        if(dComboWrapper->Get_NumUnusedShowers()       > 5)                                                                         locCutFlag_NumUnusedShowers = true;
        ...
        if(locMissingP4_KinFit.P()                     > 0.6)                                                                       locCutFlag_MissingMomentum  = true;
        ...
        dFlatTreeInterface->Fill_Fundamental<Int_t>("L1TriggerBits",         dComboWrapper->Get_L1TriggerBits());
...
void DSelector_piminus_p_4He_recon::Finalize(void)
{
	// CALL THIS LAST
