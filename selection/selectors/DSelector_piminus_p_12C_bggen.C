...
class DSelector_piminus_p_12C_bggen : public DSelector
{
	public:

		DSelector_piminus_p_12C_bggen(TTree* locTree = NULL) : DSelector(locTree){}
		virtual ~DSelector_piminus_p_12C_bggen(){}
	...
	private:
        ...
        TH1F* dHistThrownTopologies_After;
        TH1F* dHistThrownParticles_After;

	ClassDef(DSelector_piminus_p_12C_bggen, 0);
};

void DSelector_piminus_p_12C_bggen::Get_ComboWrappers(void)
{
...
void DSelector_piminus_p_12C_bggen::Init(TTree *locTree)
{
	// SET OUTPUT FILE NAME
	dOutputFileName          = "";
	dOutputTreeFileName      = "";
	dFlatTreeFileName        = "flattree_piminus_p_12C_bggen.root";
	dFlatTreeName            = "flattree_piminus_p_12C_bggen";
...
Bool_t DSelector_piminus_p_12C_bggen::Process(Long64_t locEntry)
{
	// CALL THIS FIRST
	...
	for(UInt_t loc_i = 0; loc_i < Get_NumCombos(); ++loc_i)
	{
        ...
        TString locThrownPositive, locThrownNegative, locThrownParticle;
        ...
        for(UInt_t loc_j = 0; loc_j < Get_NumThrown(); ++loc_j)
        {
            ...
            if (loc_j == dPiMinusWrapper->Get_ThrownIndex())
            {
                ...
                if      (dThrownWrapper->Get_PID() == Proton)
                    locThrownNegative += "p^{+}";
                else if (dThrownWrapper->Get_PID() == AntiProton)
                    locThrownNegative += "p^{-}";
                else if (dThrownWrapper->Get_PID() == PiPlus)
                    locThrownNegative += "#pi^{+}";
                else if (dThrownWrapper->Get_PID() == PiMinus)
                    locThrownNegative = "#pi^{-}";
                else if (dThrownWrapper->Get_PID() == KPlus)
                    locThrownNegative += "K^{+}";
                else if (dThrownWrapper->Get_PID() == KMinus)
                    locThrownNegative = "K^{-}";
                else if (dThrownWrapper->Get_PID() == Positron)
                    locThrownNegative = "e^{+}";
                else if (dThrownWrapper->Get_PID() == Electron)
                    locThrownNegative = "e^{-}";
                else if (dThrownWrapper->Get_PID() == MuonPlus)
                    locThrownNegative = "#mu^{+}";
                else if (dThrownWrapper->Get_PID() == MuonMinus)
                    locThrownNegative = "#mu^{-}";
                else
                    cout << "Other negative: " << dThrownWrapper->Get_PID() << endl;
            }
            else if (loc_j == dProtonWrapper->Get_ThrownIndex())
            {
        ...
        if(locMissingP4.P()                            > 5.0)                                                                locCutFlags[5] = true;
        ...
        // if(locMissingP4.Minus()                        < 0.6  || locMissingP4.Minus()            > 1.2)                      locCutFlags[8] = true;
...
void DSelector_piminus_p_12C_bggen::Finalize(void)
{
    // FORMAT CUSTOM HISOTGRAMS
	dHistThrownTopologies_After->GetXaxis()->LabelsOption(">");
    dHistThrownTopologies_After->GetXaxis()->SetRangeUser(1, 10);
    dHistThrownTopologies_After->Scale(1/dHistThrownTopologies_After->Integral());
