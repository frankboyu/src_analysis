...
class DSelector_phi_he_4He_data : public DSelector
{
	public:

		DSelector_phi_he_4He_data(TTree* locTree = NULL) : DSelector(locTree){}
		virtual ~DSelector_phi_he_4He_data(){}
	...
	private:
		...
		DChargedTrackHypothesis* dHeliumWrapper;
	...
	ClassDef(DSelector_phi_he_4He_data, 0);
};

void DSelector_phi_he_4He_data::Get_ComboWrappers(void)
{
	...
	dHeliumWrapper    = static_cast<DChargedTrackHypothesis*>(dStep0Wrapper->Get_FinalParticle(2));
}

void DSelector_phi_he_4He_data::Init(TTree *locTree)
{
	// SET OUTPUT FILE NAME
	dOutputFileName                             = "";
	dOutputTreeFileName                         = "";
	dFlatTreeFileName                           = "flattree_phi_he_4He_data.root";
	dFlatTreeName                               = "flattree_phi_he_4He_data";
...
Bool_t DSelector_phi_he_4He_data::Process(Long64_t locEntry)
{
	// CALL THIS FIRST
	...
	for(UInt_t loc_i = 0; loc_i < Get_NumCombos(); ++loc_i)
	{
		...
		Int_t locHeliumTrackID  = dHeliumWrapper->Get_TrackID();
		...
		TLorentzVector locHeliumP4   = dHeliumWrapper->Get_P4_Measured();
...
void DSelector_phi_he_4He_data::Finalize(void)
{
	// CALL THIS LAST
