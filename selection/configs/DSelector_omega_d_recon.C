...
void DSelector_omega_d_recon::Init(TTree *locTree)
{
    ...
    dHist_ChiSqPerNDF_Before        = new TH2F("ChiSqPerNDF_Before",        ";M_{3#pi} (GeV)            ;#Chi^2/NDF",           500,    0.0,    5.0,    20,     0.0,    20);
    ...
    dHist_ChiSqPerNDF_After         = new TH2F("ChiSqPerNDF_After",         ";M_{3#pi} (GeV)            ;#Chi^2/NDF",           500,    0.0,    5.0,    20,     0.0,    20);
...
Bool_t DSelector_omega_d_recon::Process(Long64_t locEntry)
{
    ...
    for(UInt_t loc_i = 0; loc_i < Get_NumCombos(); ++loc_i)
	{
        ...
        dHist_ChiSqPerNDF_Before        ->Fill((locDecayingPi0P4+locPiPlusP4+locPiMinusP4).M(), dComboWrapper->Get_ChiSq_KinFit()/dComboWrapper->Get_NDF_KinFit());
        ...
        if(dPhoton1Wrapper->Get_Shower_Quality()< 0.5   || dPhoton2Wrapper->Get_Shower_Quality()< 0.5)          dComboWrapper->Set_IsComboCut(true);
        if(dDeuteronWrapper->Get_dEdx_CDC()     == 0.0)                                                         dComboWrapper->Set_IsComboCut(true);
        if((locDecayingPi0P4+locPiPlusP4+locPiMinusP4).M()                                              > 1.5)  dComboWrapper->Set_IsComboCut(true);
        ...
        dHist_ChiSqPerNDF_After         ->Fill((locDecayingPi0P4+locPiPlusP4+locPiMinusP4).M(), dComboWrapper->Get_ChiSq_KinFit()/dComboWrapper->Get_NDF_KinFit());
