...
class DSelector_phi_d_misskp_recon : public DSelector
{
public:

    DSelector_phi_d_misskp_recon(TTree* locTree = NULL) : DSelector(locTree){}
    virtual ~DSelector_phi_d_misskp_recon(){}
...
private:
    ...
    ClassDef(DSelector_phi_d_misskp_recon, 0);
};

void DSelector_phi_d_misskp_recon::Get_ComboWrappers(void)
{
	dStep0Wrapper       = dComboWrapper->Get_ParticleComboStep(0);
	dComboBeamWrapper   = static_cast<DBeamParticle*>(dStep0Wrapper->Get_InitialParticle());
    dKMinusWrapper      = static_cast<DChargedTrackHypothesis*>(dStep0Wrapper->Get_FinalParticle(0));
	dDeuteronWrapper    = static_cast<DChargedTrackHypothesis*>(dStep0Wrapper->Get_FinalParticle(1));
}

void DSelector_phi_d_misskp_recon::Init(TTree *locTree)
{
    ...
    dOutputFileName          = Form("selectedhist_phi_d_misskp_recon_%s.root", dTag.c_str());
    dOutputTreeFileName      = "";
    dFlatTreeFileName        = Form("selectedtree_phi_d_misskp_recon_%s.root", dTag.c_str());
    dFlatTreeName            = "selectedtree_phi_d_misskp_recon";
    ...
    dHist_MissingMass_Before        = new TH1F("MissingMass_Before",        ";M_{X} (GeV)               ;Events/0.01 GeV",      500,    0.0,    5.0);
    ...
    dHist_MissingMass_After         = new TH1F("MissingMass_After",         ";M_{X} (GeV)               ;Events/0.01 GeV",      500,    0.0,    5.0);
...
Bool_t DSelector_phi_d_misskp_recon::Process(Long64_t locEntry)
{
	// CALL THIS FIRST
    ...
    for(const auto& loc_combo : loc_combos)
	{
        ...
        TLorentzVector locKPlusP4       = locBeamP4 + TLorentzVector(0, 0, 0, 1.875612859) - locKMinusP4 - locDeuteronP4; // missing K+
        ...
        TLorentzVector locKPlusP4_Measured      = locBeamP4_Measured + TLorentzVector(0, 0, 0, 1.875612859) - locKMinusP4_Measured - locDeuteronP4_Measured; // missing K+
        ...
        dHist_MissingMass_Before        ->Fill((locKPlusP4_Measured).M());
        ...
        if(dDeuteronWrapper->Get_dEdx_CDC()*1e6 <= (TMath::Exp(-3.3*locDeuteronP4_Measured.P()+4.1)+2.3))       dComboWrapper->Set_IsComboCut(true);
        if(dDeuteronWrapper->Get_dEdx_ST()*1e3  <= (TMath::Exp(-1.9*locDeuteronP4_Measured.P()+2.8)+0.6))       dComboWrapper->Set_IsComboCut(true);
        ...
        if(locKPlusP4_Measured.M()              > 0.8   || locKPlusP4_Measured.M()              < 0.2)          dComboWrapper->Set_IsComboCut(true);
        if(dComboWrapper->Get_ChiSq_KinFit()/dComboWrapper->Get_NDF_KinFit()                            > 5)   dComboWrapper->Set_IsComboCut(true);
        ...
        dHist_MissingMass_After         ->Fill((locKPlusP4_Measured).M());
...
void DSelector_phi_d_misskp_recon::Finalize(void)
{
	// CALL THIS LAST
