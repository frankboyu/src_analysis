...
void DSelector_rho_d_recon::Init(TTree *locTree)
{
    ...
    dHist_ChiSqPerNDF_Before        = new TH2F("ChiSqPerNDF_Before",        ";M_{#pi^{+}#pi^{-}} (GeV)  ;#Chi^2/NDF",           500,    0.0,    5.0,    20,     0.0,    20);
    ...
    dHist_ChiSqPerNDF_After         = new TH2F("ChiSqPerNDF_After",         ";M_{#pi^{+}#pi^{-}} (GeV)  ;#Chi^2/NDF",           500,    0.0,    5.0,    20,     0.0,    20);
...
Bool_t DSelector_rho_d_recon::Process(Long64_t locEntry)
{
    ...
    for(UInt_t loc_i = 0; loc_i < Get_NumCombos(); ++loc_i)
	{
        ...
        dHist_ChiSqPerNDF_Before        ->Fill((locPiPlusP4+locPiMinusP4).M(), dComboWrapper->Get_ChiSq_KinFit()/dComboWrapper->Get_NDF_KinFit());
        ...
        dHist_ChiSqPerNDF_After         ->Fill((locPiPlusP4+locPiMinusP4).M(), dComboWrapper->Get_ChiSq_KinFit()/dComboWrapper->Get_NDF_KinFit());
