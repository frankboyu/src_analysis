...
class DSelector_phi_d_exc_recon : public DSelector
{
public:

    DSelector_phi_d_exc_recon(TTree* locTree = NULL) : DSelector(locTree){}
    virtual ~DSelector_phi_d_exc_recon(){}
...
private:
    ...
    ClassDef(DSelector_phi_d_exc_recon, 0);
};

void DSelector_phi_d_exc_recon::Get_ComboWrappers(void)
{
...
void DSelector_phi_d_exc_recon::Init(TTree *locTree)
{
    ...
    dOutputTreeFileName      = "";
    dFlatTreeFileName        = Form("selectedtree_phi_d_exc_recon_%s.root", dTag.c_str());
    dFlatTreeName            = "selectedtree_phi_d_exc_recon";
...
Bool_t DSelector_phi_d_exc_recon::Process(Long64_t locEntry)
{
	// CALL THIS FIRST
...
void DSelector_phi_d_exc_recon::Finalize(void)
{
	// CALL THIS LAST
