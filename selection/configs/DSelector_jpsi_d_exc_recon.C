...
class DSelector_jpsi_d_exc_recon : public DSelector
{
public:

    DSelector_jpsi_d_exc_recon(TTree* locTree = NULL) : DSelector(locTree){}
    virtual ~DSelector_jpsi_d_exc_recon(){}
...
private:
    ...
    TH2D* dHist_PositronKinematics_Before;
    TH2D* dHist_ElectronKinematics_Before;
    TH2D* dHist_DeuteronKinematics_Before;
    TH2D* dHist_DeuterondEdxCDC_Before;
    TH2D* dHist_DeuterondEdxST_Before;
    TH1D* dHist_InvariantMassJpsi_Before;
    ...
    TH2D* dHist_PositronKinematics_After;
    TH2D* dHist_ElectronKinematics_After;
    TH2D* dHist_DeuteronKinematics_After;
    TH2D* dHist_DeuterondEdxCDC_After;
    TH2D* dHist_DeuterondEdxST_After;
    TH1D* dHist_InvariantMassJpsi_After;
    TH2D* dHist_ChiSqPerNDF_After;

    ClassDef(DSelector_jpsi_d_exc_recon, 0);
};

void DSelector_jpsi_d_exc_recon::Get_ComboWrappers(void)
{
...
void DSelector_jpsi_d_exc_recon::Init(TTree *locTree)
{
    ...
    dOutputFileName          = Form("selectedhist_jpsi_d_exc_recon_%s.root", dTag.c_str());
    dOutputTreeFileName      = "";
    dFlatTreeFileName        = Form("selectedtree_jpsi_d_exc_recon_%s.root", dTag.c_str());
    dFlatTreeName            = "selectedtree_jpsi_d_exc_recon";
    ...
    dHist_PositronKinematics_Before = new TH2D("PositronKinematics_Before", ";P (GeV/c)                 ;#theta (deg)",         100,    0.0,    10.0,   180,    0.0,    180.0);
    dHist_ElectronKinematics_Before = new TH2D("ElectronKinematics_Before", ";P (GeV/c)                 ;#theta (deg)",         100,    0.0,    10.0,   180,    0.0,    180.0);
    ...
    dHist_InvariantMassJpsi_Before  = new TH1D("InvariantMassJpsi_Before",	";M_{e^{+}e^{-}} (GeV)      ;Events/0.01 GeV",      500,    0.0,    5.0);
    dHist_ChiSqPerNDF_Before        = new TH2D("ChiSqPerNDF_Before",        ";M_{e^{+}e^{-}} (GeV)      ;log(#chi^{2}/NDF)",    400,    0.9,    4.9,    100,    0.0,    5);
    ...
    dHist_PositronKinematics_After  = new TH2D("PositronKinematics_After",  ";P (GeV/c)                 ;#theta (deg)",         100,    0.0,    10.0,   180,    0.0,    180.0);
    dHist_ElectronKinematics_After  = new TH2D("ElectronKinematics_After",  ";P (GeV/c)                 ;#theta (deg)",         100,    0.0,    10.0,   180,    0.0,    180.0);
    ...
    dHist_InvariantMassJpsi_After   = new TH1D("InvariantMassJpsi_After",	";M_{e^{+}e^{-}} (GeV)      ;Events/0.01 GeV",      500,    2.2,    3.4);
    dHist_ChiSqPerNDF_After         = new TH2D("ChiSqPerNDF_After",         ";M_{e^{+}e^{-}} (GeV)      ;log(#chi^{2}/NDF)",    100,    0.9,    1.1,    100,     0.0,   1);
    ...
    dFlatTreeInterface->Create_Branch_Fundamental<Int_t>("ep_id");
    dFlatTreeInterface->Create_Branch_Fundamental<Int_t>("em_id");
...
Bool_t DSelector_jpsi_d_exc_recon::Process(Long64_t locEntry)
{
	// CALL THIS FIRST
    ...
    for(const auto& loc_combo : loc_combos)
	{
        ...
        dHist_PositronKinematics_Before ->Fill(locPositronP4_Measured.P(), locPositronP4_Measured.Theta()*rad_to_deg);
        dHist_ElectronKinematics_Before ->Fill(locElectronP4_Measured.P(), locElectronP4_Measured.Theta()*rad_to_deg);
        ...
        dHist_InvariantMassJpsi_Before  ->Fill((locPositronP4+locElectronP4).M());
        dHist_ChiSqPerNDF_Before        ->Fill((locPositronP4+locElectronP4).M(), TMath::Log10(dComboWrapper->Get_ChiSq_KinFit()/dComboWrapper->Get_NDF_KinFit()));
        ...
        if((locPositronP4+locElectronP4).M()    < 0.8)                                              dComboWrapper->Set_IsComboCut(true);
        if(dComboWrapper->Get_ChiSq_KinFit()/dComboWrapper->Get_NDF_KinFit()                > 1e6)  dComboWrapper->Set_IsComboCut(true);
        ...
        dHist_PhotonEnergy_After        ->Fill(locBeamP4_Measured.E(), locHistAccidWeightFactor);
        ...
        dHist_PositronKinematics_After  ->Fill(locPositronP4_Measured.P(), locPositronP4_Measured.Theta()*rad_to_deg, locHistAccidWeightFactor);
        dHist_ElectronKinematics_After  ->Fill(locElectronP4_Measured.P(), locElectronP4_Measured.Theta()*rad_to_deg, locHistAccidWeightFactor);
        dHist_DeuteronKinematics_After  ->Fill(locDeuteronP4_Measured.P(), locDeuteronP4_Measured.Theta()*rad_to_deg, locHistAccidWeightFactor);
        dHist_DeuterondEdxCDC_After     ->Fill(locDeuteronP4_Measured.P(), dDeuteronWrapper->Get_dEdx_CDC()*1e6, locHistAccidWeightFactor);
        dHist_DeuterondEdxST_After      ->Fill(locDeuteronP4_Measured.P(), dDeuteronWrapper->Get_dEdx_ST()*1e3, locHistAccidWeightFactor);
        dHist_InvariantMassJpsi_After   ->Fill((locPositronP4+locElectronP4).M(), locHistAccidWeightFactor);
        dHist_ChiSqPerNDF_After         ->Fill((locPositronP4+locElectronP4).M(), TMath::Log10(dComboWrapper->Get_ChiSq_KinFit()/dComboWrapper->Get_NDF_KinFit()), locHistAccidWeightFactor);
        ...
        dFlatTreeInterface->Fill_Fundamental<Int_t>("ep_id", locPositronTrackID);
        dFlatTreeInterface->Fill_Fundamental<Int_t>("em_id", locElectronTrackID);
...
void DSelector_jpsi_d_exc_recon::Finalize(void)
{
	// CALL THIS LAST
