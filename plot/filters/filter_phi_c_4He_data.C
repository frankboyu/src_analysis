...
int filter_phi_c_4He_data()
{
    // Open the file and get the tree
    TChain *chain = new TChain("flattree_phi_c_4He_data");
    chain->Add("/work/halld2/home/boyu/src_analysis/selection/output/flattree_phi_c_4He_data/*.root");
    TFile *output_file = new TFile("output/filteredtree_phi_c_4He_data.root", "RECREATE");
    ...
    chain->SetBranchAddress("beam_p4_meas", &BeamP4);
    chain->SetBranchAddress("kp_p4_meas", &KPlusP4);
    chain->SetBranchAddress("km_p4_meas", &KMinusP4);
    chain->SetBranchAddress("accidweight", &WeightFactor);
    ...
    // for (Long64_t i = 0; i < 100000; i++)
    {
        ...
        double y_phi                = minus_t/(2*mass_helium4*(BeamP4->E()-PhiP4->E()));
