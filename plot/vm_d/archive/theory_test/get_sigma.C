...
double dxs_weight_func(double minust_truth)
{
    double a1 = 10222.59309;
    double b1 = 19.33908;
    double a2 = 20.23543;
    double b2 = 3.33070;
    return (a1*TMath::Exp(-b1*minust_truth) + a2*TMath::Exp(-b2*minust_truth));
}
...
void fcn(int &npar, double *gin, double &f, double *par, int iflag) {
    ...
    for (size_t i = 0; i < exp_crs.size(); ++i) {
        ...
        double delta = (exp_crs[i] - theory_crs) / exp_err[i] / dxs_weight_func(-data_t[i]);
        chisq += delta * delta;
    }
